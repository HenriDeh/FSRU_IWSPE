...
export solve_greenfield
function solve_greenfield()
    ...
    for (port, coo) in ports_coordinates
        if sum(value.(port_upgrade)[port_dict[port], :]) == 0
            pop!(ports_coordinates, port)
        end
    end
    return g, consumers_dict, domestic_dict, port_dict, import_dict, export_dict, ports_coordinates, penalized_arcs
end